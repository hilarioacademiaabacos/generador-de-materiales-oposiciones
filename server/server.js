...
const app = express();
// Configuración de CORS: Permite que tu plataforma (Moodle) se comunique con el servidor
...
Responde ÚNICAMENTE con HTML completo desde <!DOCTYPE html> hasta </html>. Sin markdown.`;
  ...
  (Plantilla HTML omitida para brevedad pero se mantiene la lógica original)`;
...
Rellena TITULO_DEL_TEMA, SUBTITULO_DEL_TEMA y ARRAY_DE_PREGUNTAS_AQUI (30 objetos en formato JSON). Responde ÚNICAMENTE con HTML. Sin markdown.`;
  return "";
}
...
app.post('/generate', async (req, res) => {
  ...
  if (!process.env.GEMINI_API_KEY) return res.status(500).json({ error: 'API key no configurada.' });
  ...
  try {
    ...
    // Limpieza de bloques de código markdown (```html ... ```)
    html = html.replace(/^```html?\s*/i, '').replace(/```\s*$/, '').trim();

    // --- CORRECCIÓN AQUÍ: Eliminamos la línea que borraba el JS del test ---
    
    // Verificación básica de que recibimos un HTML
    if (!html.toLowerCase().includes('<!doctype') && !html.toLowerCase().includes('<html')) {
      throw new Error('La respuesta de la IA no es un HTML completo. Por favor, intenta de nuevo.');
    }
